$(function(){
	...
	$(".logo").on('click',function(){
		$(".loop-album").hide();
		$(".video-wrap").show();
		$(".wrapper").css({"background-image":"linear-gradient(-180deg, #676767 0%, #353535 50%, #000 100%)"});

    	scrimg = $("#list li.hove").attr('scrbg');
    	hovimg = $("#list li.hove").attr('scrhov');
		...
		$(".video-skip").show();
			...
			$(".video-pause").hide();
			$(".company-intro").hide();
			$("#list").removeClass("filt");
			$(".foot-nav").removeClass("shownav");
			// bgm
			$(".bgm").removeClass("showtime");
			musicstate=0;
			audio.pause();
			$(".mbg").hide();
	});
    ...
    //跳过
    var scrimg = $("#list li.hove img").attr('scrbg');
    var hovimg = $("#list li.hove img").attr('scrhov');
    $(".video-skip").on("click",function(){
    	video.pause();
    	$(this).hide();
    	$(".video-wrap video").addClass("animated bounceOut");
    	$(".video-pause").addClass("animated bounceOut");
